...
async function pollAnalysisResults(analysisId, fileName = '') {
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            // CORREGIDO: Uso de backticks para la interpolación
            ...
            // CORREGIDO: Math.min y variable interval bien escrita
            interval = Math.min(interval * 1.5, 8000);
            await new Promise(resolve => setTimeout(resolve, interval));
        } catch (error) {
            // CORREGIDO: message bien escrito
            showError(`Error: ${error.message}`);
            break;
        }
    }
...
function showFormattedResult(data) {
    ...
    // CORREGIDO: Sintaxis de los parámetros de reduce
    ...
    const categories = {
        ...
        // CORREGIDO: "undetected" sin la sílaba extra
    ...
    // CORREGIDOS: Errores HTML en el span del verdict, atributo title, class=""detail-item" y onclick
    updateResult(`
        <h3> Scan Report</h3>
