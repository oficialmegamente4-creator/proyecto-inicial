...
const API_KEY = "0f2aff96a02354b102ce946b97c6f0ac27b18cf09113718d33cfeaadad4feeee";

// Utility funtion to get DOM elements by ID
const getElement = id => document.getElementById(id);

// Update the result display section with given content
...
async function scanURL() {
    ...
    try {
        ...
        // Delay before polling for result
        await new Promise(resolve => setTimeout(resolve, 3000));

        showLoading("Getting scan results...");
        await pollAnalysisResults(submitResult.data.id);
    } catch (error) {
        showError(`Error: ${error.message}`); // CORREGIDO: backticks usados
    }
}
...
async function scanfile() {
    ...
    try {
        ...
        showLoading("Getting scan results...");
        // CORREGIDO: backticks en lugar de comillas simples
        ...
        // CORREGIDO: backticks y sintaxis de interpolación correcta
...
async function pollAnalysisResults(analysisId, fileName = '') {
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            // CORREGIDO: Math.min en lugar de matchMedia.min
            interval = Math.min(interval * 1.5, 8000);
            // CORREGIDO: interval en lugar de internal
...
function showFormattedResult(data) {
    ...
    // CORREGIDO: Error de sintaxis en reduce solucionado (sum, val)
    ...
    const categories = {
        ...
        // CORREGIDO: "undetected" escrito correctamente
...
// Displays a detailed modal with engine-by-engine detection result
function showFullReport(reportData) {
    ...
    const results = data.data?.attributes?.result;
